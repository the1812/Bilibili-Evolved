...
デフォルトのキーが適切でないと感じた場合、または他のプラグイン/スクリプトショートカットと競合していることがわかった場合、この<a href="https://github.com/the1812/Bilibili-Evolved/blob/preview/src/video/keymap/key-bindings.md" target="__blank">修改默认键位</a>を参照して、自分で変更することができます.`],
    ...
    ["useDarkStyle", /*html*/`夜間モードでは、ページの明るい部分が暗い色に変わり、夜間の視聴に最適な表示になります.そして多くのテーマカラーを適用します.ぜひお試しください.`],
    ["darkSchedule", /*html*/`夜間モードを使用して计画タイムを設定する.この期間を開始または終了すると、夜間モードは自動的にオンまたはオフになり、終了時刻は開始時刻より短く、翌日と見なされます.例えば<span>18:00</span>から<span>6:00</span>夜の18：00から翌日の6：00までを表しています.`],
...
注釈:「フォロー」と「レベル」アイコンはマウスでアバタ上に置くことで，ポップアップした資料カードに表示することができる.`],
    ["simplifyLiveroom", /*html*/`ライブルームで不要な内容を非表示にする
