...
各項目は、非表示にするかどうかを<span>附加功能</span>の中で個別に選択することができる.`],
    ...
    ['hideCategory', `ホームページのエリア欄を隠す、それでもエリア欄は頂欄のホームメニューからアクセスできます.`],
    ...
    ["showDeadVideoTitle", `個人センターの中に、失効した動画のタイトルとサムネイルを復元する.`],
    ["useBiliplusRedirect", `失効した動画のリンクを「BiliPlus」ページに変更して.`],
    ["deadVideoTitleProvider", `失効した動画の情報源: 「後で見る」は安定していますが、少し時間がかかります; 「BiliPlus」のスピードは速いですが、書き残していますので、しばらく開けません.`],
    ...
    ["i18nLanguage", `翻訳言語`],
    ['noLiveAutoplay', `生放送のホームページのおすすめライブルームで自動的に再生を開始することは禁止されています.`],
    ['hideHomeLive', `生放送のホームページのおすすめライブルームを隠す.`],
    ['foldComment', `情報ページにコメントを表示しているときに下部に「コメントを閉じる」ボタンを追加します.`],
    ['seedsToCoins', `追加機能に<span>瓜子换硬币</span>ボタンを追加し、クリックして1日に1回に制限されて、1コインに700銀ヒマワリの種を交換してください.`],
    ...
    ["comboLike", `「おすすめ三連」（「いいね」を長押した）のタッチサポートを有効にする.`],
