...
const entry = async (settings: ComponentSettings) => {
  ...
  const { dq, raiseEvent, getCsrf } = await import('@/core/utils')
  const { addStyle } = await import('@/core/style')
  const { Toast } = await import('@/core/toast')
  const { getJsonWithCredentials, postTextWithCredentials } = await import('@/core/ajax')
  const { default: playbackTipStyle } = await import('./playback-tip.scss')
  ...
  if (isWatchlater) {
    videoChange(() => {
      getJsonWithCredentials(`https://api.bilibili.com/x/web-interface/archive/has/like?aid=${unsafeWindow.aid}`).then(json => {
  ...
  document.body.addEventListener('keydown', e => {
    ...
    if (key in clickableKeymap && noModifyKeys) {
      ...
      video.currentTime += settings.options.jumpSeconds
      e.stopPropagation()
      e.preventDefault()
      ...
      if (isWatchlater) {
        ...
        postTextWithCredentials('https://api.bilibili.com/x/web-interface/archive/like', Object.entries(formData).map(([k, v]) => `${k}=${v}`).join('&')).then(() => {
          liked = !liked
          if (liked) {
        ...
        document.body.addEventListener('keyup', keyArgs => {
          keyArgs.preventDefault()
          fireEvent('mouseup', keyArgs)
          if (likeClick) {
            fireEvent('click', keyArgs)
          }
        }, { once: true })
      }
      ...
      if (key === '>' || key === 'ArrowUp'.toLowerCase()) { // 天堂制造
        video.playbackRate = playbackRates.find(it => it > video.playbackRate)
          || playbackRates[playbackRates.length - 1]
        showPlaybackTip(video.playbackRate)
      } else if (key === '<' || key === 'ArrowDown'.toLowerCase()) { // 时间减速
        video.playbackRate = [...playbackRates].reverse().find(it => it < video.playbackRate)
          || playbackRates[0]
        showPlaybackTip(video.playbackRate)
      } else if (key === '?') { // 重置速度
