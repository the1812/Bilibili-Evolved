...
<details><summary>导出设置文件命名格式说明</summary>

自定义导出设置的文件命名格式，基本规则同 `文件命名格式`.

可以使用的变量有:
- `n`: 本体的名称，示例: `Bilibili Evolved`
- `v`：本体的版本号，示例: `v2.xx.x`
- `V`：带有构建分支等 tag 信息的详细版本号
- `y`/`M`/`d`: 年/月/日
- `h`/`m`/`s`/`ms`: 时/分/秒/毫秒

默认的格式是 `[n]_[v] [y]-[M]-[d] [h]-[m]-[s]`.

</details>

<details><summary>导出组件选项文件命名格式说明</summary>

自定义导出组件选项时的文件命名格式，基本规则同 `导出设置文件命名格式`.

新增的变量:
- `c`: 组件的内部名称

默认的格式是 `[c]_[n] [y]-[M]-[d] [h]-[m]-[s]`.
