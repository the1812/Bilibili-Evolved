...
export const forEachContextMenu = async (action: {
  ...
  const player = await SpinQuery.select('.bilibili-player') as HTMLElement
  if (!player) {
    return
  }
  const playerObserver = Observer.childList(player, () => {
