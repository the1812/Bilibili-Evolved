...
export async function convertToAss(xml) {
    ...
    try {
        ...
        config.font = dq('.bilibili-player-video-danmaku-setting-right-font .bui-select-result').innerText;
        config.alpha = parseFloat(dq('.bilibili-player-setting-opacity .bui-bar').style.transform.replace(/scaleX\(([\d\.]+)\)/, '$1'));
        config.duration = (() => {
        ...
        config.blockTypes = (() => {
            ...
            for (const [type, value] of Object.entries(blockValues)) {
                if (dq(type).classList.contains('disabled')) {
        ...
        if (config.bottomMarginPercent === 0 && dq('.bilibili-player-video-danmaku-setting-left-preventshade input').checked) // 无显示区域限制时要检查是否开启防挡字幕
         {
            config.bottomMarginPercent = 0.15;
        }
        config.bold = dq('.bilibili-player-video-danmaku-setting-right-font-bold input').checked;
    }
    catch (error) {
        // The default config
        config = {
...
export async function downloadDanmaku(ass, timeout) {
    ...
    dq('#download-danmaku>span').innerHTML = '下载弹幕';
    ...
    widget: {
      ...
      <button
        ...
        success: () => {
            ...
            dq('#download-danmaku').addEventListener('click', (e) => {
                if (e.target !== link) {
                    const timeout = setTimeout(() => dq('#download-danmaku>span').innerHTML = '请稍侯...', 200);
