...
async function rpc(getResponse, batch = false) {
    try {
...
async function getRpc(rpcParam, batch = false) {
...
async function postRpc(rpcParam, batch = false) {
    const { option, host, methodName } = getOption();
    return await rpc(async () => {
...
export async function sendRpc(params, batch = false) {
    const option = settings.aria2RpcOption;
