...
export const component = defineComponentMetadata({
  ...
  entry: async ({ metadata }) => {
    ...
    const showUploadTime = (relist: VideoPageCard[], forceUpdate: boolean = false, formatString: string = '') => {
      if (!formatString) {
      ...
      for (const key in formatMap) {
        if (new RegExp(`(${key})`).test(format)) {
          format = format.replace(
            ...
            RegExp.$1.length === 1
              ? formatMap[key]
              : ('00' + formatMap[key]).substr(('' + formatMap[key]).length))
        }
      }
      return format
    }

    addComponentListener(`${metadata.name}.formatString`, (value: string) => {
