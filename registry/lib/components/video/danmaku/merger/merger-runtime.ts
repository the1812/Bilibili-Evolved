...
export const initDanmakuMerger = (): MergerCleanup => {
  ...
  const registerMergerMenu = (name, fn) => {
    ...
    try { pageWin().__dmMergerLoadedVersion = DM_MERGER_VERSION; } catch (e) { }
    ...
    // Bilibili API 封装（Wave 1 bilibili.ts，保持原调用方兼容）
    const API = {
    ...
    function loadPartModeState(bvid) {
        try {
            const raw = getStorage().get(getPartModeStorageKey(bvid));
    ...
    function savePartModeState(bvid, state) {
        if (!bvid) return;
        try {
            getStorage().set(getPartModeStorageKey(bvid), JSON.stringify(state));
            getStorage().trackKey(getPartModeStorageKey(bvid));
        } catch (e) { }
    }
    ...
    registerMergerMenu('🗑️ 清除所有弹幕合并记忆', () => {
        const dmKeys = getStorage().listMergerKeys().filter(k => k.startsWith('dm_merger_store_'));
        ...
        if (confirm(`确定要清除所有 ${dmKeys.length} 个视频的记录吗？`)) {
            dmKeys.forEach(key => getStorage().delete(key));
            location.reload();
        }
    });
