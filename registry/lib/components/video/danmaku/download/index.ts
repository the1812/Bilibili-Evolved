    ...
    setup: ({ addData }) => {
      addData('downloadVideo.assets', async (assets: DownloadVideoAssets[]) => {
        ...
        assets.push({
          name: 'downloadDanmaku',
          displayName: '下载弹幕',
          getAssets: async (infos, instance: {
            ...
            const results = await Promise.allSettled(infos.map(async info => {
            ...
            const success = results.filter(it => it.status === 'fulfilled') as PromiseFulfilledResult<PackageEntry>[]
