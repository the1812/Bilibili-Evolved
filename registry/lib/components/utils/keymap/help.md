...
在快捷键设置中, 每个动作都配备了 3 套按键, 分别是默认按键, 预设按键和自定义按键 (优先级依次升高). 默认按键由动作自带, 预设按键将根据选择的预设而变化, 如果都不是所希望的按键, 则可以添加自定义按键. 除了键盘按键外, 也可以绑定鼠标中键和侧键.
...
- 主键盘数字区和小键盘可以细分指定, 例如主键盘的 <kbd>0</kbd> 是 `digit0`, 小键盘的 <kbd>0</kbd> 是 `numpad0`
- 鼠标中键、后退侧键和前进侧键分别写作 `mouseMiddle`、`mouseBack` 和 `mouseForward`, 也可以和 `shift` / `ctrl` / `alt` / `meta` 组成组合键
- b 站自带有一些快捷键, 修改本组件提供的相同动作快捷键不会影响 b 站自带的快捷键. 如果希望全部屏蔽, 可以开启组件的 `屏蔽 B 站播放器原生快捷键` 选项; 如果希望屏蔽其他按键, 可以安装 `快捷键扩展 - 无动作` 插件, 并将无动作绑定到希望屏蔽的快捷键上. 例如希望用 `C` 键投币且不希望自带的 `W` 键触发投币, 则应该在组件里配置投币为 `c`, 无动作为 `w`
...
addData('keymap.actions', (actions: Record<string, KeyBindingAction>) => {
  actions.watchlater = {
    ...
    run: context => {
      ...
      return clickElement(
        '.video-toolbar .ops .watchlater, .more-ops-list .ops-watch-later, .video-toolbar-module .see-later-box',
        context,
      )
    },
  }
