...
const getPastebinUrl = async (str: string, config: ConfigDataType) => {
  ...
  if (/^Bad API request,/.test(response)) throw response
...
export const MPV_Ex: DownloadVideoOutput<ConfigDataType> = {
  ...
  description: '多文件时格式选择 flv，使用前请先阅读 <a href="https://github.com/Asukaaaaaa/tricks/blob/main/Bilibili-Evolved%20mpv-ex%20%E6%8F%92%E4%BB%B6.md" target="blank">README</a>',
  runAction: async (action, instance) => {
    ...
    if (action.isSingleVideo) {
      ...
      // 多文件生成 .m3u 播放列表
      const m3u = action.infos.reduce((acc, v, i) => {
        return acc + `#EXTINF:-1,${v.input.title}\n${v.fragments[0].url}\n` // ? 暂不支持外挂音轨，默认取第一个资源的链接
      }, '#EXTM3U\n')
      try {
        const playlist = (await getPastebinUrl(m3u, instance)).replace(/^(https:\/\/pastebin.com)\/(.*)/, '$1/raw/$2?.m3u')
